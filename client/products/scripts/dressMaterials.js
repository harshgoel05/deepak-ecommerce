...
window.addEventListener('load',async function (){
    ...
    for (const wishButton of wishButtons) {
       wishButton.addEventListener('click', async function (event) {
            ...
            const response = await fetch('https://testing1.thestrategybook.com/deepak-ecommerce/server/api/products/dressMaterials/retrieve.php?productid=' + product_id);
            const json = await response.json();
            const object = json.data;
            console.log(object);
            const data = {
                "product_category": "dressMaterials",
