...
function append(cartItem) {
  ...
  root.innerHTML =
    ...
    </div> <div class="row"> <div class="col-8 d-flex justify-content-between remove_wish"> <a class="rem" colors=${cartItem.selected_colors} productid=${cartItem.productid} category =${cartItem.product_category} size=${cartItem.selected_size} onclick="deleteBlock(${cartItem.productid},${cartItem.selected_quantity},this)"><i class="fas fa-trash-alt"></i> REMOVE<br />ITEM</a> <a class="wish" productid=${cartItem.productid} colors=${cartItem.selected_colors} category =${cartItem.product_category} size=${cartItem.selected_size} onclick="sendToCart(${cartItem.productid},${cartItem.selected_quantity},this)"><i class="fas fa-heart" style="margin-right: 5px"></i>MOVE TO<br />CART </a> </div> <div class="col-4 d-flex justify-content-end price_money"> <h3>Rs <span id=${itemval}>${constAmt} </span></h3> </div> </div> </div> </div> </div>`;
}
...
const deleteBlock = async (id, quantity, item) => {
  ...
  fetch("https://shreedeepaksarees.com/server/api/user/wishlist/remove.php", requestOptions)
...
const sendToCart = async (id, quantity, item) => {
  console.log(id, quantity, item);
  id = item.attributes.productid.value;
  var color = item.attributes.colors.value;
  color = secondWord(color);
...
const deleteBlockForWishList = async (id, quantity, item) => {
  console.log('Deleting for wishlist');
  id = item.attributes.productid.value;
